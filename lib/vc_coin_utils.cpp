        ...
        if (distSq <= distThresholdSq) {
            ...
            if (goldBlobs[i].xc < EDGE_MARGIN || goldBlobs[i].yc < EDGE_MARGIN || 
                ...
                if (coinType >= 4 && coinType <= 6) {
                    ...
                    // Remove edge coin logging message
                    return true;
                }
 ...
 * 
 * This function is called from main.cpp after ProcessFrame to add
 * high-quality text labels to the detected coins
 * 
 * @param frame Original OpenCV frame to label
 * @param excludeList Exclusion list containing coin positions
 * @param coinData Array containing coin type information
 */
void AddCoinLabelsCV(cv::Mat &frame, int *excludeList) 
{
    if (excludeList == NULL) return;
    
    const int MAX_COINS_TO_LABEL = 20;
    ...
    // Process each potential coin in the exclusion list
    for (int i = 0; i < MAX_COINS; i++) {
        int x = excludeList[i * 2];
        int y = excludeList[i * 2 + 1];
        
        // Skip empty entries
        if (x == 0 && y == 0) continue;
        
        // Get the coin type from the tracked coins data
        int coinType = GetLastCoinTypeAtLocation(x, y);
        
        // Skip unrecognized coins
        if (coinType <= 0 || coinType > 8) continue;
        
        // Define label based on coin type
        std::string label;
        ...
        switch (coinType) {
            case 1: // 1 cent
                label = "1c";
                labelColor = cv::Scalar(0, 80, 255); // Orange for copper
                break;
            case 2: // 2 cents
                label = "2c";
                labelColor = cv::Scalar(0, 80, 255); // Orange for copper
            ...
            case 4: // 10 cents
                label = "10c";
                labelColor = cv::Scalar(0, 215, 255); // Gold/yellow
                break;
            case 5: // 20 cents
                label = "20c";
                labelColor = cv::Scalar(0, 215, 255); // Gold/yellow
            ...
            case 7: // 1 euro
                label = "1€";
                labelColor = cv::Scalar(255, 0, 0); // Blue
                break;
            case 8: // 2 euros
                label = "2€";
                labelColor = cv::Scalar(255, 0, 0); // Blue
        ...
        cv::Size textSize = cv::getTextSize(label, fontFace, fontScale, thickness, NULL);
        
        // Calculate position (center the text on the coin)
        cv::Point textPos(x - textSize.width / 2, y + textSize.height / 2);
        
        // Draw black outline for better visibility against any background
        cv::putText(frame, label, textPos, fontFace, fontScale, cv::Scalar(0, 0, 0), thickness + 2);
        
        // Draw colored text
        cv::putText(frame, label, textPos, fontFace, fontScale, labelColor, thickness);
    }
}
